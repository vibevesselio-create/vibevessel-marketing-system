...
// MGM Canonical Database IDs - Load from script properties with environment support
...
function getDatabaseConfig() {
  ...
  const itemTypesOverride = scriptProps.getProperty('ITEM_TYPES_DB_ID');
  
  // MGM Canonical Database IDs (defaults, can be overridden via script properties)
  const canonical = {
    ...
    ITEM_TYPES: null  // Configure via script property ITEM_TYPES_DB_ID or DB_ID_{ENV}_ITEM_TYPES
  };

  if (itemTypesOverride) {
    canonical.ITEM_TYPES = itemTypesOverride;
  }
  ...
  for (const [key, value] of Object.entries(props)) {
    if (key.startsWith(envPrefix)) {
      ...
      overrides[dbName] = value;
    }
  }
  
  // Merge defaults with overrides
  const config = { ...canonical, ...overrides };
  
  // Validate UUID format
  for (const [name, id] of Object.entries(config)) {
    if (!id) {
      if (name !== 'ITEM_TYPES') {
        console.warn(`[WARN] Missing database ID for ${name}`);
      }
      continue;
    }
...
const CONFIG = {
  ...
  // DEPRECATION TRACKING: Monitor usage of deprecated databases/{id} endpoint
  // These counters are incremented when fallback to deprecated endpoint is used
  // Review logs with 'DEPRECATED_ENDPOINT_FALLBACK' prefix to identify migration needs
  DEPRECATED_ENDPOINT_TRACKING: {
    enabled: true,
    logLevel: 'warn', // 'debug' | 'warn' | 'error'
    counters: {
      resolveDatabaseToDataSourceId: 0,
      fetchDatabaseSchema: 0,
      filterDbPropsToExisting: 0,
      getWorkspaceDbNotionDatabaseId: 0
    }
  },
  ...
  LOGGING: {
    ...
    ENV_PROP: 'LOG_ENV',
    DEFAULT_SCRIPT_NAME: 'DriveSheetsSync',
    DEFAULT_ENV: 'DEV',
...
function registerLocalDriveFolderInNotion_(UL) {
  try {
    ...
    const props = {
      'Name': { title: [{ type: 'text', text: { content: `Google Drive For Desktop - ${clientName}` } }] },
...
function findFolderEntryByPath_(dbId, dsId, absolutePath, UL) {
  try {
    const queryResource = dsId ? `data_sources/${dsId}/query` : `databases/${dbId}/query`;
    const body = {
      filter: {
...
function getOrCreateFolderInNotion_(driveFolder, UL) {
  try {
    ...
    // Also try to find by folder ID if we have a "Folder ID" property
    if (!existingId) {
      try {
        const queryResource = foldersDsId ? `data_sources/${foldersDsId}/query` : `databases/${FOLDERS_DB_ID}/query`;
        const body = {
          filter: {
            or: [
              { property: 'Folder ID', rich_text: { equals: folderId } },
              { property: 'Google Drive Folder ID', rich_text: { equals: folderId } }
            ]
          },
          page_size: 1
        };
...
function getClientIdFromNotion_(clientContext, UL) {
  try {
    ...
    const queryResource = clientsDsId ? `data_sources/${clientsDsId}/query` : `databases/${CLIENTS_DB_ID}/query`;
...
function findScriptPageInNotion_(UL) {
  try {
    ...
    const queryResource = scriptsDsId ? `data_sources/${scriptsDsId}/query` : `databases/${CONFIG.SCRIPTS_DB_ID}/query`;
  ...
  _findScriptPageByScriptId_(scriptId) {
    ...
    try {
      ...
      try {
        ...
        queryResource = scriptsDsId ? `data_sources/${scriptsDsId}/query` : `databases/${scriptsDbId}/query`;
      } catch (resolveErr) {
      ...
      const namePropNames = ['Name', 'Script Name', 'Title'];
...
function validateSingleInProgressInvariant_(dbId, UL) {
  ...
  try {
    ...
    // Use already resolved dsId for querying
    const queryResource = dsId ? `data_sources/${dsId}/query` : `databases/${dbId}/query`;
...
function getInProgressTasks_(dbId, UL) {
  try {
    ...
    // Resolve database_id to data_source_id for querying
    const dsId = resolveDatabaseToDataSourceId_(dbId, UL);
    const queryResource = dsId ? `data_sources/${dsId}/query` : `databases/${dbId}/query`;
    const results = notionFetch_(queryResource, 'POST', { filter });
    const tasks = [];
...
function resolveDatabaseToDataSourceId_(databaseId, UL) {
  ...
  // DEPRECATION NOTICE: This uses the databases/{id} endpoint which may be deprecated
  try {
    // Log deprecation warning when using databases endpoint
    UL?.debug?.('DEPRECATED_ENDPOINT_USAGE: resolveDatabaseToDataSourceId_ using databases/{id} endpoint', {
      databaseId: normalizedId,
      originalId: databaseId,
      purpose: 'Resolving database_id to data_source_id',
      deprecationNote: 'The databases/{id} GET endpoint may be deprecated in future Notion API versions'
    });
    ...
    if (!sources || sources.length === 0) {
      UL?.warn?.('Database has no data sources array', {
        databaseId: normalizedId,
        originalId: databaseId,
...
function fetchDatabaseSchema_(dbId, UL) {
  ...
  try {
    ...
    // DEPRECATION WARNING: databases/{id} endpoint may be deprecated in future API versions
    // This fallback is tracked for visibility and future migration planning
    try {
      UL?.warn?.('DEPRECATED_ENDPOINT_FALLBACK: Using databases/{id} endpoint as fallback', {
        databaseId: dbId,
        primaryError: err,
        endpoint: `databases/${dbId}`,
        recommendation: 'Plan migration to data_sources endpoint only',
        deprecationNote: 'The databases/{id} GET endpoint may be deprecated in future Notion API versions'
      });
      ...
      return { ok: true, props, titleName, usedDeprecatedFallback: true };
    } catch (e2) {
...
function getWorkspaceDbNotionDatabaseId_(UL) {
  ...
  try {
    // Try data_sources endpoint first (preferred)
    const dsId = resolveDatabaseToDataSourceId_(dbId, UL);
    if (dsId) {
      notionFetch_(`data_sources/${dsId}`, 'GET');
      UL?.debug?.('Workspace DB accessible via data_sources endpoint', { dbId, dsId });
      return dbId;
    }
    // Fall back to legacy databases endpoint with deprecation warning
    UL?.warn?.('DEPRECATED_ENDPOINT_FALLBACK: Checking Workspace DB via databases/{id} endpoint', {
      dbId,
      recommendation: 'Plan migration to data_sources endpoint only',
      deprecationNote: 'The databases/{id} GET endpoint may be deprecated in future Notion API versions'
    });
    notionFetch_(`databases/${dbId}`, 'GET');
    UL?.debug?.('Workspace DB accessible via legacy endpoint', { dbId });
    return dbId;
  } catch (e) {
    const errorMsg = String(e);
    const stack = e.stack || 'No stack trace available';
...
function _filterDbPropsToExisting_(dbId, props) {
  try {
    ...
    let db = null;
    let usedDeprecatedFallback = false;
    try {
    ...
    if (!db) {
      // DEPRECATION WARNING: Using legacy databases/{id} endpoint as fallback
      // This fallback usage is logged for visibility and future migration planning
      console.warn('DEPRECATED_ENDPOINT_FALLBACK: _filterDbPropsToExisting_ using databases/{id} endpoint', {
        dbId,
        deprecationNote: 'The databases/{id} GET endpoint may be deprecated in future Notion API versions'
      });
    ...
    const exist = new Set(Object.keys(db.properties || {}));
    const out = {};
    for (const [k, v] of Object.entries(props || {})) if (exist.has(k)) out[k] = v;
    return out;
  } catch (_) { return {}; }
}
...
function ensureDbFolder_(parentId, ds) {
  ...
  try {
    const UL = getUnifiedLogger_();
    getOrCreateFolderInNotion_(parent, UL);
  } catch (e) {
      ...
      if (lock.tryLock(10000)) {
        try {
          ...
          if (matchingFolders.length === 0) {
            ...
            try {
              const UL = getUnifiedLogger_();
              getOrCreateFolderInNotion_(newFolder, UL);
            } catch (e) {
        ...
        if (matchingFolders.length === 0) {
          // Still no folder after waiting - create without lock (fallback)
          console.warn(`[WARN] Lock timeout, creating folder without lock protection`);
          const newFolder = parent.createFolder(expectedName);
          console.log(`[INFO] Created new database folder (fallback): ${expectedName}`);
          return newFolder;
        }
      }
    } catch (lockErr) {
      console.warn(`[WARN] Lock error: ${lockErr}, attempting folder creation without lock`);
      // Re-check one more time
      matchingFolders = findMatchingFolders_();
      if (matchingFolders.length === 0) {
        const newFolder = parent.createFolder(expectedName);
        console.log(`[INFO] Created new database folder (lock error fallback): ${expectedName}`);
        return newFolder;
      }
    }
  }
          ...
          if (CONFIG.EXECUTION_LOGS_DB_ID) {
            try {
              ...
              if (execLogsDsId) {
                properties[propName] = {
                  relation: {
                    database_id: CONFIG.EXECUTION_LOGS_DB_ID,
